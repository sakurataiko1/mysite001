...
//    QTextStream g_out_obj(&outfile);
    ...
    for(int m=0; m < drawMeshList.size(); m++){
        ...
        //座標定義  openGL表示座標に変換。　　および、objファイル書き込み記述作成
    ...
    //-start- objファイル書き込み
    qDebug() << "[DEBUG]-start- miWidget-drawSurfaceOfMesh() objfile-write" + QDateTime::currentDateTime().toString("hh:mm:ss");
    QString outfileDir =  QFileInfo(QFile(voxfilePath)).absolutePath() + "/objfile_" + QFileInfo(QFile(voxfilePath)).fileName();//2021.06.xx-01 for-obj
    QString outfilePath = outfileDir + "/" + QFileInfo(QFile(voxfilePath)).fileName() + "_" + QString::number(mateNo) + ".obj"; //
    if(QFile(outfilePath).exists()) { QFile(outfilePath).remove(); }
    myfunction *m_myfunction = new myfunction;
    m_myfunction->fileWriteForWindows(outfilePath, "WriteOnly", g_objList);
    qDebug() << "[DEBUG]-end- miWidget-drawSurfaceOfMesh() objfile-write" + QDateTime::currentDateTime().toString("hh:mm:ss");
    //-end- objファイル書き込み
}
...
void miWidget::func_getPointOfMesh_new(int in_i, int in_j, int in_k, int surfaceNo, int input_MateNo, int in_surfaceCnt) //kuroda　もとのgetPointOfMesh 関数から変更
{
    ...
    if(surfaceNo == 2){ //glFront 2021.06.xx-01変更  checkMate関数では通常座標（Y:奥行　手前が-,奥が+)としているが、ここではopenGL用（glZ前が-, 奥が+で描画するための変更）
        ...
        //func_objfile_write(input_MateNo, in_surfaceCnt, tmpmsg ,vertexD, vertexA, vertexC, vertexB); //objファイル書き込み
        func_objfile_write_plane(vertexD, vertexA, vertexC, vertexB, input_MateNo, in_surfaceCnt, tmpmsg, "Front"); //objファイル書き込み内容をQStringListに保持
    }
    ...
    if(surfaceNo == 1){ //glBack 2021.06.xx-01変更  checkMate関数では通常座標（Y:奥行　手前が-,奥が+)としているが、ここではopenGL用（glZ前が-, 奥が+で描画するための変更）
        ...
        func_objfile_write_plane(vertexH, vertexE, vertexG, vertexF, input_MateNo, in_surfaceCnt, tmpmsg, "Back"); //objファイル書き込み内容をQStringListに保持
    }
    ...
    if(surfaceNo == 3){ //Right
        ...
        func_objfile_write_plane(vertexC, vertexB, vertexG, vertexF, input_MateNo, in_surfaceCnt, tmpmsg, "Right"); //objファイル書き込み内容をQStringListに保持
    ...
    if(surfaceNo == 4){ //Left
        ...
        func_objfile_write_plane(vertexD, vertexA, vertexH, vertexE, input_MateNo, in_surfaceCnt, tmpmsg, "Left"); //objファイル書き込み内容をQStringListに保持
    ...
    if(surfaceNo == 5){ //Top
        ...
        func_objfile_write_plane(vertexD, vertexC, vertexH, vertexG, input_MateNo, in_surfaceCnt, tmpmsg, "Top"); //objファイル書き込み内容をQStringListに保持
    ...
    if(surfaceNo == 6){ //Bottom
        ...
        func_objfile_write_plane(vertexA, vertexB, vertexE, vertexF, input_MateNo, in_surfaceCnt, tmpmsg, "Bottom"); //objファイル書き込み内容をQStringListに保持
...
void miWidget::func_objfile_write_plane(QVector3D in_vertex1, QVector3D in_vertex2, QVector3D in_vertex3, QVector3D in_vertex4, int input_matNum, int in_surfaceCnt, QString in_msg1, QString in_surfaceStr) //objファイル書き込み
{
    //-start- objファイル作成前準備
...
//        return;
//    }
//    QTextStream g_objList(&outfile1);
    //-end- objファイル作成前準備
    ...
    //v 1.000000 -1.000000 1.000000
    g_objList << "# " + in_msg1;
    g_objList << "o Plane." + QString::number(in_surfaceCnt);
    //objでは、(x,y,z) 水平：Xは小さいほうから、奥行Zは大きい方から。高さYはx,z に合わせて。
    g_objList << "v " + QString::number(in_vertex1.x()) + " " + QString::number(in_vertex1.y()) + " " + QString::number(in_vertex1.z()) ; //objの場合↑とは描き方が違うので注意すること　objテキストファイルに書き出すは4頂点だけ
    g_objList << "v " + QString::number(in_vertex2.x()) + " " + QString::number(in_vertex2.y()) + " " + QString::number(in_vertex2.z()) ; //頂点順番はこれから修正する
    g_objList << "v " + QString::number(in_vertex3.x()) + " " + QString::number(in_vertex3.y()) + " " + QString::number(in_vertex3.z()) ;
    g_objList << "v " + QString::number(in_vertex4.x()) + " " + QString::number(in_vertex4.y()) + " " + QString::number(in_vertex4.z()) ;
    //vt,vn,s行：固定
    g_objList << "vt 0.000000 0.000000" ;
    g_objList << "vt 1.000000 0.000000" ;
    g_objList << "vt 1.000000 1.000000" ;
    g_objList << "vt 0.000000 1.000000" ;
    ...
    g_objList << "s off";
    //f行：可変。Planeごとにカウントアップ必要。
    //g_objList << "f 1/1/1 2/2/1 4/3/1"; //D A B
    //g_objList << "f 4/3/1 3/4/1 1/1/1"; //B C D
    QList<int> vList,vtList;
    vList << 1 << 2 << 4 << 4 << 3 << 1;
    vtList << 1 << 2 << 3 << 3 << 4 << 1;
    for(int i=0; i < vList.count(); i++ ){
    ...
    QString tmpstr;
    g_objList << tmpstr.asprintf("f %d/%d/%d %d/%d/%d %d/%d/%d",
                                 vList.at(0), vtList.at(0), in_surfaceCnt + 1,
                                 vList.at(1), vtList.at(1), in_surfaceCnt + 1,
                                 vList.at(2), vtList.at(2), in_surfaceCnt + 1);
    g_objList << tmpstr.asprintf("f %d/%d/%d %d/%d/%d %d/%d/%d",

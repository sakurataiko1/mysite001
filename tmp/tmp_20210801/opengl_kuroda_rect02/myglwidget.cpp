...
//    colors   = m_miWidget->func_get_voxGraffic(g_voxfilepath , "color");
    //返り値を一度に取得   //std::tie(vertices, colors) = m_miWidget->func_get_voxGraffic(g_voxfilepath, gval_MainWindow_uiTableListMat, "vertex");

    func_getPointOfMesh_fromStartEndPoint(); //関数実行して、vertices, colorsを取得
...
//2021.08.xx-01 1Meshごとでなく、面の始点・終点単位で描く
void MyGLWidget::func_getPointOfMesh_fromStartEndPoint() //1Meshごとでなく、面の始点・終点単位で描く
{
    int meshsize_x = 20; //[DEBUG]
    int meshsize_y = 20; //[DEBUG]
    int meshsize_z = 20; //[DEBUG]

    //QString zahyofilePath = "C:/kuroda/work/00_Data_vox/DEBUG_openGL/surfacelist.txt";
    QString zahyofilePath = g_voxfilepath;
    myfunction *m_myfunction = new myfunction;
    QStringList zahyoList = m_myfunction->readTextFileAll(zahyofilePath);

    for(int n=0; n < zahyoList.size(); n++){
        QString oneline = zahyoList.at(n).trimmed(); //改行削除
        QStringList onelineList = zahyoList.at(n).split(",");
        if(onelineList.size() < 8){ continue; } //処理スキップ
        int input_MateNo = onelineList.at(0).toInt();
        int surfaceNo = onelineList.at(1).toInt();
        int xStart = onelineList.at(2).toInt();
        int yStart = onelineList.at(4).toInt(); //openGL座標:y = 通常座標:z
        int zStart = onelineList.at(3).toInt(); //openGL座標:z = 通常座標:y
        int xEnd = onelineList.at(5).toInt();
        int yEnd = onelineList.at(7).toInt(); //openGL座標:y = 通常座標:z
        int zEnd = onelineList.at(6).toInt(); //openGL座標:z = 通常座標:y
        qDebug() << "[DEBUG]func_getPointOfMesh_fromStartEndPoint()" << onelineList;

        //外部ファイル読み込みで　始点・終点を取得　→　openGL として面単位で描く座標に変換
...
//        QVector3D vertexH = QVector3D(nowOP_x , nowOP_y + meshsize_y, nowOP_z - meshsize_z);
        ...
        int yWidth = yEnd - yStart;
        int zWidth = zEnd - zStart;
        QVector3D vertexA = QVector3D(xStart, yStart, zStart);
        QVector3D vertexB = QVector3D(xStart + xWidth, yStart, zStart);
        QVector3D vertexC = QVector3D(xStart + xWidth, yStart + yWidth, zStart);
        QVector3D vertexD = QVector3D(xStart , yStart + yWidth, zStart);
        QVector3D vertexE = QVector3D(xStart, yStart, zStart - zWidth);
        QVector3D vertexF = QVector3D(xStart + xWidth, yStart, zStart - zWidth);
        QVector3D vertexG = QVector3D(xStart + xWidth, yStart + yWidth, zStart - zWidth);
        QVector3D vertexH = QVector3D(xStart , yStart + yWidth, zStart - zWidth);
    ...
    //-end- for(int n=0, n<zahyolist.size, n++)
